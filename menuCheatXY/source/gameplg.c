...
// Offsets are XY in this file (i hope i got it right)

u8 *PK6_LOCATION_1 = (u8*)0x081FEBA0;
u8 *PK6_LOCATION_2 = (u8*)0x081FF744;
u16 *TID = (u16*)0x08C79C3C;
u16 *SID = (u16*)0x08C79C3E;
...
void cloneSlotOneToSlots(int numberOfSlots) {
    int i;
    int theBase = 0x08C861C8;
    int theLength = 0xE8;
    for(i = 0; i < numberOfSlots; i++) {
        memcpy((u8*) (i * 0xE8 + 0x08C861C8), (u8*) (0x08C861C8), theLength);
    }
}
...
void cloneBoxOneToBoxTwo() {  // Copy all Pokemon from Box 1 into Box 2 (forward)
    memcpy((u8*) (0x1B30 + 0x08C861C8), (u8*) (0x08C861C8), 0x1B30);
}

void cloneBoxTwoToBoxOne() {  // Copy all Pokemon from Box 2 back into Box 1 (backward)
    memcpy((u8*) (0x08C861C8), (u8*) (0x1B30 + 0x08C861C8), 0x1B30);
}

void shinyB1S1() {
    PK6 pkm;
    PK6_LOCATION_1 = (u8*)0x08C861C8;
...
void EncConstToNickB1S1() {  // test that changes the nickname of slot 1 pkm to the encryption constant (maybe)
    ...
    if(getCurrentPokemon(&pkm)) {
        char charbytes[12];
        xsprintf(charbytes, "%X", READU32(0x08C861C8));
        charbytes[8] = 0x00;
        charbytes[9] = 0x00;
        charbytes[10] = 0x00;
        charbytes[11] = 0x00;
